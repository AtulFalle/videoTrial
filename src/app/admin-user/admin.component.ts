...
export class AdminComponent implements OnInit {
  studies: any[] = [{ value: 'All', viewValue: 'All' }];
  sites: any[] = [{ value: 'All', study: 'All', viewValue: 'All' }];
  ...
  ngOnInit(): void {
    ...
    this.store$
      .select(VideoTrialStoreSelectors.getAllRoles)
      .subscribe((roles) => {
        ...
        if (roles.Studies) {
          ...
          Object.keys(roles.Studies).map((study) => {
            ...
            roles.Studies[study].map((site: any) => {
              this.sites.push({ value: site, study, viewValue: site });
            });
          });
        }
      });
    ...
    this.store$
      .select(VideoTrialStoreSelectors.getAllUsers)
      .subscribe((res) => {
        ...
        res.map((user) => {
          if (user.accountStatus == 'Requested') {
            ...
            Object.keys(user.selectedRole).length === 0
          ) {
            const tempUser = {
  ...
  getSite = () => {
    if (this.selectedSudy === 'All') {
      ...
      return this.sites.filter(
        (site) => site.study === 'All' || site.study === this.selectedSudy
      );
    }
  }
  //  getUsers = () => {
  ...
  getExistingUsers = () => {
    if (this.selectedSudy === 'All' && this.selectedSite === 'All') {
      ...
      return this.existingUserList.filter(
        (user) => user.study === this.selectedSudy
      );
    } else if (this.selectedSudy == 'All' && this.selectedSite !== 'All') {
      return this.existingUserList.filter(
        (user) => user.site === this.selectedSite
      );
    } else {
      return this.existingUserList.filter(
        (user) =>
          user.study === this.selectedSudy && user.site === this.selectedSite
      );
    }
  }
  ...
  getUserSites = () => {
    const allUserRequests: UserApproval[] = [...this.existingUserList];
    const selectedRequest = this.existingUserList[this.selectedIndex];
    const filteredArr = allUserRequests.filter(
      (userRequest) =>
        userRequest.emailId === selectedRequest.emailId &&
        userRequest.site !== selectedRequest.site
    );
    ...
    if (this.selectedUserSudy === '') {
      ...
      return filteredSites.filter(
        (site) => site.value !== 'All' && site.study === this.selectedUserSudy
      );
    }
  }
  onUserStudySelect = () => {
    const filteredSites = this.sites.filter(
      (site) => site.value != 'All' && site.study == this.selectedUserSudy
    );
    this.selectedUserSite =
      filteredSites.length > 0 ? filteredSites[0].value : '';
  }
  ...
  rejectUserRequest(emailId: string): void {
    const encodedEmail = emailId.replace('@', '%40');
    this.store$.dispatch(
      VideoTrialStoreActions.updateUserRoles({
  ...
  updateUserRole(id: string, objectId: string): void {
    this.selectedIndex = -1;
    ...
    if (this.userList[index].selectedRole) {
      ...
      const newSite = {
        id:
          this.selectedUserSudy + 'site-id-' + (Math.random() * 100).toFixed(3),
  ...
  updateUserStatus(id: string, status: string, objectId: string): void {
    this.selectedIndex = -1;
    const index = this.userList.findIndex((ele) => ele.objectId === objectId);
    const tempSelectedRole: any = {};
    if (this.userList[index].selectedRole) {
  ...
  addNewRoles = () => {
    const index = this.userList.findIndex(
      (ele) => ele.email === this.userNewRole
    );
    const tempSelectedRole: any = {};
    if (this.userList[index].selectedRole) {
      Object.keys(this.userList[index].selectedRole).map((study) => {
  ...
  geSitesForNewRole = () => {
    const allUserRequests: UserApproval[] = [...this.existingUserList];
    const selectedRequest = this.existingUserList.find(
      (user) => user.emailId === this.userNewRole
    );
    if (selectedRequest) {
      const filteredArr = allUserRequests.filter(
        (userRequest) =>
      ...
      if (this.newRoleSudy === '') {
        ...
        return filteredSites.filter(
          (site) => site.value !== 'All' && site.study === this.newRoleSudy
        );
      }
    }
    return [];
  ...
  geSitesAssignRole = () => {
    if (this.assignedSudy === '') {
      ...
      return this.sites.filter(
        (site) => site.value !== 'All' && site.study === this.assignedSudy
      );
    }
  }

  removeSiteFromNewRole = (id: number) => {
    this.newRoleSitesList = this.newRoleSitesList.filter(
      (role) => role.id !== id
    );
  }
  removeSiteAssignedSites = (id: number) => {
    this.assignedSitesList = this.assignedSitesList.filter(
      (role) => role.id !== id
    );
  }
}
